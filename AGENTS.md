...
判定为误报及理由）」的小结。**这份记录有两条硬规则**（M11 用 13 次闸门调用换来的）：

1. **只追加已发生的事。** 不得引用尚未产出的轮次/跑次，不得对尚未完成的修复
   写「已修」。每一节在它所记的那一轮**结束之后**才写入。
   M11 里同一形态犯了 4 次，每次都让记录「声称自己被审过、实际没有」。
2. **终止条件按「修的是什么」定，不按「修了没有」定。** 一轮闸门若**只**在
   记录的流水账里发现问题（轮次、agent 数、HEAD、计数、内部指针），
   修完**不触发新一轮**；若修复触及任何**规范性文本**（`AGENTS.md`、`skills/**`、
   `docs/create-project.md`，或任何一条「发现 → 处置」的实质判断），
   则**必须**触发新一轮。
   没有这一条，「写下第 N 轮的结果」本身就制造出未审状态，闸门永远停不下来
   —— M11 后半程的成本几乎全在这里。

**闸门在中途发现的 pre-existing 契约缺陷，默认记进 `BACKLOG.md` 或下一个
单独的里程碑**，不在本轮就地改。理由不是它不重要，而是契约改动会同时重新
武装两道闸门：M11 就是这么从「改两行 skill 笔记」变成 13 次闸门调用的。
...
闸门 B 若报了问题，就在同一分支上继续修、继续推，重跑到干净为止。
