...
- 不做盘中/实时刷新。只有 EOD（收盘）一个数据点。
  ...
  陈旧告警按**交易日**而非日历日计算 —— 有了 `trading_sessions.ordinal`，
  这个判断是结构性正确的，不靠「减一减日期」。
- 不做用户系统、不做自选股。前端**纯只读** ——
...
- `is_estimated` 落库，前端对估计值加**虚线下划线 + tooltip**（§10.4 的直接应用）。
...
- **`is_estimated` 必须有生命周期，不能只有 `default true`。**
  ...
  把settled 的事实标成「估计」是另一种不诚实，而且会让虚线到处都是、从而失去意义。
...
代价为零：§1.2 已排除历史展示，§10.2 唯一的消费者是当天表格里的倒计时芯片。
...
| 端点 | 给什么 | 供给哪个参数 |
|---|---|---|
| `Ticker.calendar` | 下一次财报日（常为估计）、下一次除息日 | `days_to_next_earnings` / `days_to_next_dividend` |
...
metrics:
  ...
  - id: event_distances
    fn: event_distances            # 依赖 symbol_events 表，不依赖收盘价（§3.6）
    core: true
...
1. **日历**：`pandas_market_calendars` 的 schedule 判断今天是否为交易日，
   ...
   日历代码用 **`XNAS`**（universe 全是纳斯达克上市）而非 `XNYS` ——
   两者的假日与半日市实际完全一致，所以这是个明示的选择而非笔误。
...
前端 header 的「数据截至 <date>」+ 超过 1 个交易日未更新变黄条仍然保留 ——
...
| **尊重 429 / `Retry-After`** | 收到 429 一律先读 `Retry-After`；没有该头则指数退避 + 抖动（2s → 4s → 8s，上限 60s） |
...
实现为一个统一的 sanitizer（`df.replace([np.nan, np.inf, -np.inf], None)`）+ 一条单元测试。
...
固定 0.5pp 的切点在前一种状态下**永不触发**，在后一种状态下**一直触发**
—— 警告最响的时候恰恰是排名分得最开的时候。
...
- **响应式（按表格优先的新布局）**：
  ...
  - <768px：表格转为卡片列表（金融表格强行横滚是最糟的移动端体验）；
    三强横条纵向堆叠为三行；**α/β 行内展开改为卡片内的可折叠区**
    （这是删掉个股页后新增的那一块工作，M7 要算进去）。
...
| **M7** | 前端成品 | 全池表格+sparkline（主体，α/β 明细行内展开）、三强横条、α–β 散点、方法论页、**密码 middleware + /login**、双主题、响应式表格→卡片、四种非理想态 | 移动端可用；Lighthouse ≥ 95；四种状态可手动触发演示；**403 白屏态必须真的实现**（§12 #9 第 4 条） | **2.0d** |
...
| 13 | 事件距离参数 | **已定：加**财报/分红的前后距离共 4 个（§3.6），单位为**日历日** | 新增 `symbol_events` 表（第六张公开表）；抓取**不可批量**（逐标的 17 次）故改为**周频**刷新；**事件列只写最新一行、历史行全为 NULL**（§3.5(3)），于是不存在前视泄露也不需要就此写免责声明 |
...
| **财报日是估计值却被当成事实** | 用户按一个会挪的日期判断事件风险 | §3.6(2)：`is_estimated` 落库，前端虚线下划线区分；临近（≤10 天）改为每日刷新 |
