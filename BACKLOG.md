...
- **陈旧黄条的探测延迟从 ~1 天放宽到 7 个日历日**（2026-09-22，§10.6）。
  旧判据「>1 个交易日」每天收盘后到管道写入之间、以及整个周末假日都会亮，
  数据完全正常 —— 一条天天出现的告警等于没有告警，所以这个交换是划算的：
  **用探测延迟换可信度**。要注意它换掉的到底是什么：
  自动探测器（`invariants.sql` 的「管道不得静默停摆」，容忍 1 个 session）
  没有变，但它由 `keepalive.yml` 跑 —— 而「GitHub 因 60 天无活动停用全部
  workflow」那一类故障会**同时**掐掉 keepalive 和 daily，此时没有任何自动
  断言会执行，黄条就是最后一道防线，延迟也就实打实是 7 天。
  真正的解是下面那条 `HEALTHCHECK_URL`（外部的 dead-man's switch 才能探测
  「什么都没跑」），配上之后这条延迟就不再重要。
...
- **Next 15.x 仍带着有漏洞的 `postcss`**（`npm audit` 说要升到 16.x 才干净）。
  ...
  不塞进这个 PR。
- **全池表格同时挂着三个名字。** 冻结表头那一轮给可滚动的框加了
  `role="region" aria-label="全池速览（…）"`，而 `page.tsx` 的 sr-only `<h2>`
  与 `<caption>` 已经各有一个名字；首页因此出现同名嵌套 landmark，
  `/states` 里那个名字还是错的（那张表在「部分标的缺失」的演示块下，
  却自称「全池速览」）。caption 早就是同样的硬编码写法，所以不是这一轮引入的。
  想收敛成一份真相：给 caption 一个 id，region 改用 `aria-labelledby` 指过去。
  不阻塞 —— 不违反 §10.4 的可访问性条款。
...
- **精简 `docs/reviews/M11.md`。** 第 9 轮的审查者指出它约 40% 是自我叙事
  （「这一轮真正的教训」整节 22 行、对自己出错率的评语、同一段故事讲两遍），
  而 `AGENTS.md` 要求的「发现 → 处置」本体只占一部分。**记录比被记录的东西还长。**
  骨架（需求出处 / 做成了什么 / 各轮发现表 / 判定为不修的 / 验证）保留，其余可砍。
- **`docs/reviews/M9.md` 没有闸门 B 记录**（全文零个 `codex`）。要么那一道没跑，
  要么跑了没记 —— 两种都违反约定，需要查清并补记或标注。
- **`AGENTS.md` 有约六成篇幅在讲「怎么审自己」**（review 闸门 + Git 工作流）。
  流程文本正在挤占约定文本，值得一次瘦身。
